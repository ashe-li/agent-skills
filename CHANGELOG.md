...
- **新增對抗性 E2E 測試 `scripts/tests/test_plan_run_e2e_adversarial.py`**：故意製造 plan-run 應該撐得住的六種狀況，驗證 runner 與 Stop hook 的反應。全程用 subprocess 從外部呼叫 CLI 與 `hook-stop`，不 import 內部函式，每個情境各用一個假 `$HOME`。六種狀況：C1 少了腳本（`Command:` 引用的工具、state 檔、runner 本身，runner 這條走真的 `plan-run-stop.sh` wrapper，並附一個補回 runner 就會派工的對照）；C2 某個 step 永遠不前進（STUCK）；C3 出現範圍外指令（sandbox 規則要寫進 reason、`log-out-of-scope` 要保存原文、checkpoint 只能有筆數）；C4 step 失敗；C5 執行到一半 context 重置（換 session 後的 lease 交接、`next --resume`、已完成的 step 不重做）；C6 `Requires-Approval` 關卡。斷言先拿還沒有這些功能的 dev 跑過一次，當下就會 PASS 的空轉斷言已經拿掉；另外把 STUCK 延後觸發、核准關卡失效、範圍外指令沒記錄、checkpoint 沒寫這四種錯誤各自套到實作的複本上，E2E 都會判紅。共用 harness 放在 `plan_run_e2e_support.py`。`PLAN_RUN_E2E_RUNNER` 可以改測別的 runner 複本；設了 `PLAN_RUN_E2E_LOG_DIR` 才會替每個情境輸出一份 Markdown 和一份 JSONL 的 run log，預設不寫任何檔案。**驗不到的部分**：模型會不會真的拒絕範圍外指令、會不會主動去記錄（C3 只驗到規則文字有進 reason、記錄真的有寫入，這兩條規則在工具呼叫層沒有強制力）；多個 session 同時搶 pointer lock 的競態（C5 的交接是一個接一個跑的）；Claude Code 本身怎麼處理 exit 2 和 `systemMessage`（依官方文件推論，沒有實際觀察）。
