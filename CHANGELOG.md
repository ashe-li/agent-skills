...
### Added
- **`/eli5` 白話解釋 skill（新增）**：Anthropic 員工 Thariq Shihipar 分享的 `/eli5`（`anthropics/claude-plugins-community`，原版僅一句「像對完全不懂的人解釋，用一個大圖少字的 HTML artifact」）的繁體中文版。兩種模式：對話中「白話解釋」「我看不懂」「X 是什麼」→ 純文字四段（一句話類比 → 要點 → 類比失效點 → 技術細節可選）；`/eli5 <主題>`／「圖解」「畫給我看」→ 一頁 HTML 大圖少字（inline SVG 畫面，Artifact 工具或 `.eli5/<slug>.html`）。內建與 `speak-human-tw`（去 AI 味改寫）分工、對外文件不掛「白話」label、簡化不得改事實三道邊界。動機：session log 「白話」請求 ×74 為最高頻糾正，先給白話省一輪往返。
