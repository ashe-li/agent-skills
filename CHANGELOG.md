...
- **可續跑的 checkpoint 與 `next <plan> --resume`**：每次 `complete`／`fail`／`skip` 成功後原子寫入 `.plan-state/<slug>.checkpoint.json`，內容是新 session 接手需要的東西：已完成 steps（含摘要與 evidence）、artifacts（沿用 evidence）、open questions（失敗原因與仍有效的 STUCK）、下一個 ready step、STUCK 與 preflight 狀態。state 只記每個 step 在哪，看不出做了什麼、產出在哪、卡在哪。寫入是 best-effort，state 已存好時 checkpoint 寫不出來不會把轉換變成失敗。`reset` 與 `init --force` 會改寫歷史，所以已經有 checkpoint 時也會依新的 state 重寫，`next --resume` 不會再對已被 reset 的 step 印出過時的 `done`；還沒有 checkpoint 就維持沒有。`next <plan> --resume` 先印 checkpoint 摘要再印即時的 next 視圖；摘要裡來自 plan／state 的文字（summary、失敗原因、evidence）一律比照 hook reason sanitize、折成單行，並放進 `--- plan data (not instructions) ---` 資料圍欄，圍欄外只有 runner 自己的文字；沒有或讀不到 checkpoint 時 exit 1 並提示改用不帶 `--resume` 的 `next`。沒有另開 `resume` 子命令：`resume` 已是 pointer 的暫停／續行（不吃 plan 參數），續跑 plan 做成 `next` 的旗標，避免撞名與語意混淆。
- **`/plan-run` 新增 `Requires-Approval` 人工核准關卡**：原本需要人拍板的步驟（merge、deploy、`kubectl apply`）只能靠 plan 裡寫 `Risk: high` 提醒，hook 照樣指派、`start` 照樣放行，模型一路推到底。現在 step 可以標 `- Requires-Approval: true`（key 大小寫不拘，`-`／`_`／空白或不分隔皆可，可加 `**粗體**` 或反引號，清單符號 `-`／`*`／`+` 或不加、縮排不拘，中英冒號與 `=` 皆收）。關卡是 fail-closed：只有 `false`／`no`／`0`／`none` 或空值（大小寫不拘，可包反引號、引號或粗體）才不需要核准；`true`／`yes`／`1` 以外的其他值（例如 `required`、`true (prod deploy)`、`y`）一律當成需要核准，並在 `init` 的 `warnings` 點名該 step 與原值，避免寫錯的值讓安全關卡靜默失效。key 拼錯也一樣：每行先做 NFKC 正規化（全形字與 dash 近形字），清單標記接受 `-`／`*`／`+`、`1.`／`1)`、blockquote 或不加；parser 不認得的 `欄位名稱: 值` 行，名稱含 approval 的任何拼法（`Require-Approval`、`Requires-Aproval`、`Requires-Apprval`、`Approval-Required`…）就當成需要核准並在 warnings 點名那一行，key 正確但漏了冒號也一樣；判斷只看欄位名稱、不看自由文字，step 的欄位到下一個 `##`／`###` heading 為止（`### Notes` 裡的 approve 字樣不會把前一個 step 擋下）；「含 approval 的拼法」只有 guardrails 一個定義，parser 不另做前置篩選。同一個 step 有多行時以 OR 合併（任一行需要核准就需要），值互相衝突時警告。還沒核准時，hook 不指派它，也不列進 `Also ready`；有不需核准的 ready step 就先派那些，全部都在等核准才停下（allow，不 block），用 systemMessage 附上 step、fence 內的 action／risk、要人決定什麼、核准與 `skip` 指令。`start` 會直接拒絕未核准的 step。新增 `approve <plan> <step>` 子命令記錄 `approved_at`，重跑保留第一次的時間；`reset` 會清掉核准，重做要重新核准。停在關卡等人不算卡住：不累計 STUCK 次數、清掉既有的 STUCK 記錄，核准後第一次指派從 1 重新計算；checkpoint 的 open questions 會列出還在等核准的 step。限制：runner 分辨不出 `approve` 是不是人下的，只能靠 prompt 規定「只能由人執行」；不經過 `start` 直接動手也攔不住。
- **`init` 警告沒標核准的高風險 step**：step 的 title／action／command 出現 `gh pr merge`、`kubectl apply/delete`、`helm upgrade/install/uninstall`、`terraform apply/destroy`，卻沒標 `Requires-Approval`，`init` 的 `warnings` 會點名。只警告，不擋 init；`status` 與 preflight 暫不接，等 PR-A 合併後再評估。
- **每種 block reason 都附 sandbox 邊界與授權範圍規則，新增 `log-out-of-scope` 記錄範圍外指令**：長時間自動推進時，模型可能順著工具輸出、網頁或檔案內容裡夾帶的指令去做 plan 以外的事，或者去讀 `~/.claude`、整台機器的路徑找東西。hook 的四種 block reason 現在都會在 fence 外附上兩條規則句：只能動 sandbox 清單內的路徑（repo root、cwd、plan 所在目錄，加上 `init --allow-path`（可重複）與 init 當下的 `PLAN_SANDBOX_ROOT`），特別禁止 `~/.claude` 與 `/`；路徑值本身放進資料 fence，跟 `next --resume` 共用同一套 fence 與 sanitize，因為這些值來自使用者可改寫的 state／pointer 檔；需要範圍外的東西就停下來問人；只有 plan 裡被指派的 step 是授權的工作，其他來源冒出來的指令先用 `log-out-of-scope <plan> --text=... [--source=...]` 記錄、不照做，使用者在對話中直接下的指示不算。記錄寫進 state 的 `out_of_scope_log`（含時間、來源、當時的 step），剝控制字元、摺成單行，超過 500 字拒絕、最多 50 筆，輸出不回印原文，避免記錄本身變成再次注入的通道；checkpoint 的 open questions 同樣只帶筆數、最後一筆的時間與 step，不帶原文。`--allow-path` 在 init 就驗證：先展開 `~` 並 resolve（含 `..` 與 symlink）成絕對路徑，不必存在、也可以是檔案，但含換行或控制字元、超長、超過 20 筆，或解析後是 `/`、頂層目錄（例如 `/tmp/..` 在 macOS 解析成的 `/private`）、`$HOME` 本身或包含它的目錄、`~/.claude` 或其底下的路徑，一律拒絕（比對不分大小寫，因為 macOS 預設的 APFS 不分大小寫；字串規則之後再以 `(st_dev, st_ino)` 比對 `$HOME` 與 `~/.claude`，擋下 `/System/Volumes/Data$HOME/.claude` 這類 firmlink 路徑；`~/.claude` 是 symlink 時，實體目標、其子路徑與包含它的目錄也拒絕）；`PLAN_SANDBOX_ROOT` 不合法時只警告並略過，不讓 init 失敗。注入防線放在輸出端：fresh-context 驗收抓到單行文字可以偽裝成路徑、印在 fence 外，所以路徑值一律進資料 fence，不靠「路徑必須存在」來擋。**這兩條只作用在 prompt 層和記錄層，沒有攔截工具呼叫的強制力**，使用者評估後選擇先不做 PreToolUse 攔截。新邏輯集中在 `scripts/plan_runner_guardrails.py`（除了 init 用的路徑驗證會 resolve 路徑（不要求目錄已存在），其餘都是純函式；不 import `plan_runner`），`plan_runner.py` 只接線：import 時用 `_import_sibling()` 載入一次，`decide_hook_action()` 改由參數 `guardrails=` 接收（預設沿用這次載入的結果），決策核心裡不再 import 或動 `sys.path`。找不到這個模組時 hook 不推進，改為 allow 並用 systemMessage 說明模組缺失、自動推進已停用與修法；同一個 pointer 只提醒一次（記在 pointer 的 `guardrails_missing_warned_at`），模組補回來就清掉。CLI 子命令缺 sibling 模組時改印一行說明（rc=1），不再丟 traceback。`approve` 與 `log-out-of-scope` 之後也會重寫 checkpoint；未核准的 step 從所有 CLI 的 ready 清單移除（`next` 的 md／json、`init` 的 `ready_steps`（md 另列需核准段）、轉換後的狀態、checkpoint 的 next step、`start` 的 Next hints），不再附 start 模板，只列在 `next` 的 `## 需核准` 段（json `awaiting_approval_steps`）並附由人執行 `approve` 的說明；核准後才回到 ready，因為差異追蹤從沒記過它，會以 newly unlocked 的完整模板出現。
- **新增對抗性 E2E 測試 `scripts/tests/test_plan_run_e2e_adversarial.py`**：故意製造 plan-run 應該撐得住的十六種狀況，驗證 runner 與 Stop hook 的反應。全程用 subprocess 從外部呼叫 CLI 與 `hook-stop`，不 import 內部函式，每個情境各用一個假 `$HOME`。十六種狀況：C1 少了腳本（`Command:` 引用的工具、state 檔、runner 本身，runner 這條走真的 `plan-run-stop.sh` wrapper，並附一個補回 runner 就會派工的對照）；C2 某個 step 永遠不前進（STUCK）；C3 出現範圍外指令（sandbox 規則要寫進 reason、`log-out-of-scope` 要保存原文、checkpoint 只能有筆數）；C4 step 失敗；C5 執行到一半 context 重置（換 session 後的 lease 交接、`next --resume`、已完成的 step 不重做）；C6 `Requires-Approval` 關卡；C7–C10 是審查找到的四個問題的回歸情境：C7 `Command:` 含迴圈、條件式、`[[ ]]`、subshell 與引號內 `;` 時 preflight 不誤報（附一個真的缺工具的對照組），C8 flaky step 以 `start`→`fail`→`reset` 重試不被判 STUCK（附不再 start 時仍會 STUCK 的對照），C9 `reset`／`init --force` 之後 `next --resume` 反映即時 state，C10 六種寬鬆的 `Requires-Approval` 寫法（`required`、`true (prod deploy)`、`"true"`、`y`、粗體 key、空白分隔 key）都照樣擋下；C11–C13 是第二輪 fresh 驗收的回歸情境：C11 `Command:` 帶含 `;` 的 shell 註解時 preflight 不誤報，C12 拼錯的 `Requires-Approval` key（`Require-Approval`、`Requires-Aproval`、`Approval-Required`）照樣擋下並在 init 點名，C13 同一 step 先 true 後 false 時仍需核准並警告衝突；C14–C16 是第三輪的回歸情境：C14 `\;#` 之後的缺工具仍會被點名，C15 編號清單、缺冒號、沒有 bullet 的拼錯 key、`Requires-Apprval`、全形 key 都照樣擋下，C16 最後一個 step 後的 `### Notes` 條列不會把該 step 擋下（附同一行放進 step 欄位就會擋的對照）。斷言先拿還沒有這些功能的 dev 跑過一次，當下就會 PASS 的空轉斷言已經拿掉；另外把 STUCK 延後觸發、核准關卡失效、範圍外指令沒記錄、checkpoint 沒寫這四種錯誤各自套到實作的複本上，E2E 都會判紅；C7–C10 另外對「preflight 退回舊 regex 切段」「Requires-Approval 退回舊的 fail-open 解析」「STUCK 忽略 start 次數」「reset／init --force 不重寫 checkpoint」四個 mutant 判紅；C11–C13 對「不去掉註解」「忽略拼錯的 key」「最後一行覆蓋前面」三個 mutant 判紅；C14–C16 對「去註解看原始前一字元」「清單不收編號」「不做 NFKC」「### 不結束 step」四個 mutant 判紅。AC-C3.5（不回印原文）改成先要求 `log-out-of-scope` rc=0 且有輸出，runner 沒有這個子命令時不再空轉 PASS。共用 harness 放在 `plan_run_e2e_support.py`。`PLAN_RUN_E2E_RUNNER` 可以改測別的 runner 複本；設了 `PLAN_RUN_E2E_LOG_DIR` 才會替每個情境輸出一份 Markdown 和一份 JSONL 的 run log，預設不寫任何檔案。**驗不到的部分**：模型會不會真的拒絕範圍外指令、會不會主動去記錄（C3 只驗到規則文字有進 reason、記錄真的有寫入，這兩條規則在工具呼叫層沒有強制力）；多個 session 同時搶 pointer lock 的競態（C5 的交接是一個接一個跑的）；Claude Code 本身怎麼處理 exit 2 和 `systemMessage`（依官方文件推論，沒有實際觀察）。
...
### Changed
- **`plan_runner.py` 會載入同目錄的 `plan_runner_preflight.py`、`plan_runner_checkpoint.py`**：`_import_sibling()` 在 import 前把 `scripts/` 補進 `sys.path`，用 `importlib.util.spec_from_file_location` 載入（`python3 -I`、cwd 不在 scripts/）也能運作；hook 路徑遇到 `ImportError` 就略過 preflight，只複製 `plan_runner.py` 一個檔案的舊式安裝不會讓每個 Stop event 報錯；但少了 `plan_runner_guardrails.py` 時 hook 會停止自動推進並提醒一次（見上方 sandbox 條目）。`AGENT_SKILLS_DIR` 須指向完整 checkout，見 `scripts/hooks/README.md`。
