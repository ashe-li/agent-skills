...
- **每種 block reason 都附 sandbox 邊界與授權範圍規則，新增 `log-out-of-scope` 記錄範圍外指令**：長時間自動推進時，模型可能順著工具輸出、網頁或檔案內容裡夾帶的指令去做 plan 以外的事，或者去讀 `~/.claude`、整台機器的路徑找東西。hook 的四種 block reason 現在都會在 fence 外附上兩條規則句：只能動 sandbox 清單內的路徑（repo root、cwd、plan 所在目錄，加上 `init --allow-path`（可重複）與 init 當下的 `PLAN_SANDBOX_ROOT`），特別禁止 `~/.claude` 與 `/`；路徑值本身放進資料 fence，跟 `next --resume` 共用同一套 fence 與 sanitize，因為這些值來自使用者可改寫的 state／pointer 檔；需要範圍外的東西就停下來問人；只有 plan 裡被指派的 step 是授權的工作，其他來源冒出來的指令先用 `log-out-of-scope <plan> --text=... [--source=...]` 記錄、不照做，使用者在對話中直接下的指示不算。記錄寫進 state 的 `out_of_scope_log`（含時間、來源、當時的 step），剝控制字元、摺成單行，超過 500 字拒絕、最多 50 筆，輸出不回印原文，避免記錄本身變成再次注入的通道；checkpoint 的 open questions 同樣只帶筆數、最後一筆的時間與 step，不帶原文。`--allow-path` 與 `PLAN_SANDBOX_ROOT` 在 init 就驗證：必須解析成存在的目錄，含換行或控制字元、超長、超過 20 筆、或解析後是 `/`，一律拒絕（fresh-context 驗收抓到單行文字可以偽裝成路徑、印在 fence 外）。**這兩條只作用在 prompt 層和記錄層，沒有攔截工具呼叫的強制力**，使用者評估後選擇先不做 PreToolUse 攔截。新邏輯集中在 `scripts/plan_runner_guardrails.py`（除了 init 用的路徑驗證會檢查目錄是否存在，其餘都是純函式；不 import `plan_runner`），`plan_runner.py` 只接線：import 時用 `_import_sibling()` 載入一次，`decide_hook_action()` 改由參數 `guardrails=` 接收（預設沿用這次載入的結果），決策核心裡不再 import 或動 `sys.path`。找不到這個模組時 hook 不推進，改為 allow 並用 systemMessage 說明模組缺失、自動推進已停用與修法；同一個 pointer 只提醒一次（記在 pointer 的 `guardrails_missing_warned_at`），模組補回來就清掉。`approve` 與 `log-out-of-scope` 之後也會重寫 checkpoint；未核准的 step 從所有 CLI 的 ready 清單移除（`next` 的 md／json、`init` 的 `ready_steps`、轉換後的狀態、checkpoint 的 next step），不再附 start 模板，只列在 `next` 的 `## 需核准` 段（json `awaiting_approval_steps`）並附由人執行 `approve` 的說明；核准後才回到 ready，因為差異追蹤從沒記過它，會以 newly unlocked 的完整模板出現。
