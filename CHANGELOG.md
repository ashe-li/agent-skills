...
- **`/plan-run` 新增 `Requires-Approval` 人工核准關卡**：原本需要人拍板的步驟（merge、deploy、`kubectl apply`）只能靠 plan 裡寫 `Risk: high` 提醒，hook 照樣指派、`start` 照樣放行，模型一路推到底。現在 step 可以標 `- Requires-Approval: true`（key 大小寫不拘，`-`／`_`／空白或不分隔皆可，可加 `**粗體**`，中英冒號皆收）。關卡是 fail-closed：只有 `false`／`no`／`0`／`none` 或空值（大小寫不拘，可包反引號、引號或粗體）才不需要核准；`true`／`yes`／`1` 以外的其他值（例如 `required`、`true (prod deploy)`、`y`）一律當成需要核准，並在 `init` 的 `warnings` 點名該 step 與原值，避免寫錯的值讓安全關卡靜默失效。還沒核准時，hook 不指派它，也不列進 `Also ready`；有不需核准的 ready step 就先派那些，全部都在等核准才停下（allow，不 block），用 systemMessage 附上 step、fence 內的 action／risk、要人決定什麼、核准與 `skip` 指令。`start` 會直接拒絕未核准的 step。新增 `approve <plan> <step>` 子命令記錄 `approved_at`，重跑保留第一次的時間；`reset` 會清掉核准，重做要重新核准。停在關卡等人不算卡住：不累計 STUCK 次數、清掉既有的 STUCK 記錄，核准後第一次指派從 1 重新計算；checkpoint 的 open questions 會列出還在等核准的 step。限制：runner 分辨不出 `approve` 是不是人下的，只能靠 prompt 規定「只能由人執行」；不經過 `start` 直接動手也攔不住。
