...
### Added
- **`rules/non-neutral-verification.md`（新規則，預設不 symlink 常駐）**：非中性驗證的三條紀律——**驗證的價值不在「綠」，在你能不能讓它「紅」**。①**還原步驟本身要驗**：`git stash push <paths>` 對**已 commit、工作區無修改**的路徑**靜默什麼都沒收**（不報錯、不回非零），於是「修改前」那輪跑的其實是修好的程式碼，看到「綠→綠」卻以為是「紅→綠」；而且因為沒東西可 stash，緊接的 `git stash pop` 會彈出 **stack 頂端別人的 entry**（stash stack 全 repo 共用、worktree 也共用），在無關檔案造成 `UU` 衝突。改用 `git checkout <ref> -- <明確路徑>` 取得 before、`git checkout HEAD -- <同樣路徑>` 還原。**出現 0 紅時不要往下走**；`grep -c <新符號>` 只能當輔助（改動若是「移除某符號」則修正後也是 0，該檢查失效），最可靠的證明始終是那個「紅」。②**驗收環境要有鑑別力**：假綠的第二種來源不是測試邏輯，是**驗收環境缺了讓 bug 得以成立的前提**（典型：bug 只在某 feature flag 開啟時發生，而 preview/staging 沒帶那個 flag）→ 修正前的程式碼在該環境輸出相同，綠由環境造成。做法是**在驗收環境本身跑一次對照組**，仍綠就代表該環境驗不了這件事、綠要撤回；flag-gated 改動動手前先 `grep -nE "NEXT_PUBLIC_[A-Z_]*=" .github/workflows/{build-production,deploy-*,preview-*}.yaml` 比對各環境 build-args，差異就是「哪個環境驗得了」的答案。③**用「指紋」提早識破**：挑一個同時受該前提影響、但與受測改動無關的可觀測值放進驗收清單第一項（例：flag 開→`og:image` 是 `img.vocus.cc/<sig>/w:1200/f:jpg`、關→`resize-image.vocus.cc`），它比被驗的東西**更早**暴露環境問題。連帶：發現「驗收環境缺 prod 有的 flag」要單獨記成缺口（代表該類改動在該環境**永遠**驗不了），但補 flag 常有連帶設定，別補一半把環境弄壞。動機：2026-08-25 單一 session 內同類失敗踩三次——(1) stash 靜默無效導致 **11/11 假綠**，改用 checkout 重跑真實結果是 **9 紅→全綠**；(2) preview env 缺 `NEXT_PUBLIC_OG_IMAGINARY_TO_IMGPROXY` 導致 PASS 全數撤回，改在本地帶 flag 跑 production build 並補對照組才拿到有鑑別力的結果；(3) 同輪把 `og:image` 的 host 當 flag 指紋放進斷言，直接自證環境正確，避免重蹈第 2 點。三次的測試邏輯都是對的，壞的都在「還原」與「環境」。
