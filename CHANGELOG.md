...
- **每種 block reason 都附 sandbox 邊界與授權範圍規則，新增 `log-out-of-scope` 記錄範圍外指令**：長時間自動推進時，模型可能順著工具輸出、網頁或檔案內容裡夾帶的指令去做 plan 以外的事，或者去讀 `~/.claude`、整台機器的路徑找東西。hook 的四種 block reason 現在都會在 fence 外附上兩條規則：只能動 sandbox 清單內的路徑（repo root、cwd、plan 所在目錄，加上 `init --allow-path`（可重複）與 init 當下的 `PLAN_SANDBOX_ROOT`），特別禁止 `~/.claude` 與 `/`，需要範圍外的東西就停下來問人；只有 plan 裡被指派的 step 是授權的工作，其他來源冒出來的指令先用 `log-out-of-scope <plan> --text=... [--source=...]` 記錄、不照做，使用者在對話中直接下的指示不算。記錄寫進 state 的 `out_of_scope_log`（含時間、來源、當時的 step），剝控制字元、摺成單行，超過 500 字拒絕、最多 50 筆，輸出不回印原文，避免記錄本身變成再次注入的通道；checkpoint 的 open questions 同樣只帶筆數、最後一筆的時間與 step，不帶原文。`--allow-path` 在 init 就驗證：含換行或控制字元、超長、超過 20 筆、或解析後是 `/`，一律拒絕。路徑印在 fence 外之前，會照 plan 文字的方式 sanitize。**這兩條只作用在 prompt 層和記錄層，沒有攔截工具呼叫的強制力**，使用者評估後選擇先不做 PreToolUse 攔截。新邏輯集中在 `scripts/plan_runner_guardrails.py`（純函式、不 import `plan_runner`），`plan_runner.py` 只接線，靠 `_load_guardrails()` 先把腳本目錄加進 `sys.path` 再 import，測試用 spec 依路徑載入時一樣能運作。
