...
- **`/goal` 退出 `/plan-run` driving**：`/goal` 本身就是 session-scoped 的 prompt-based Stop hook，與自寫的 script-based hook **共用同一份 block 預算**（2026-08-29 實測確認：上限是每個 turn 的 Stop 輪數而非每支 hook 的次數，兩支 blocker 各拿滿全部輪數、turn 仍在同一輪結束——所以疊 `/goal` 換不到更多步，只換到同一輪兩則互相稀釋的 reason）；且一個 session 只能一個 `/goal`，佔掉之後同 session 想跑視覺 gate 就沒得用。README 與 `/design` 的推進方式敘述同步收斂為單一案。**`figma-verify` 的 `/goal` 用法完全不動**——那是 model-evaluated 的視覺 gate，正是官方說 `/goal` 該做的事。
