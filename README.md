...
接收功能需求，自動盤點 ECC 資源，透過多個 agent 協作建立完整實作計畫。輸出至 `plans/active/<slug>.md` 供使用者確認後才進入實作。

**Pipeline:** ECC 資源盤點 → 複雜度評估 → planner → architect → plans/active/<slug>.md
...
- plan 包含 Industry & Standards Reference 表格和實作後的品質保障步驟
...
### `/notion-plan` — 有 Notion ticket 要轉為計畫

```
# Notion 需求頁面 → 自動建立 plan
...
**適合：**
- 需求寫在 Notion，想一鍵轉為 `plans/active/<slug>.md`
- 不想手動複製貼上 Notion 內容再呼叫 `/design`

**不適合：** 需求已在對話中或本地檔案（直接用 `/design`）

**Features:**
- 支援 `notion.so`、`notion.site`、短網址等多種 URL 格式
- 雙路徑策略：WebFetch（快速）→ Playwright MCP（完整 JS 渲染）
- 自動處理長頁面捲動載入、Toggle 展開、登入偵測
- 擷取內容整理為結構化 Markdown 後，自動觸發 `/design` 建立 `plans/active/<slug>.md`
...
│
├─ 準備開始新工作 ───→ /design <需求>（規劃 → plans/active/）
│
├─ 不確定 ──────────→ /assist（自動分析 + 路由）
│
...
│
├─ 需要 worktree 隔離 → /worktree create <name>（隔離開發）
│
├─ Worktree 要清理 ──→ /worktree cleanup（回收已 merge）
│
├─ 有 Notion ticket ──→ /notion-plan <URL>（抓取 → plans/active/）
│
...
| `/notion-plan` | Pending | 依賴外部 Notion 頁面，需 mock URL |
...
/notion-plan <URL>         # Notion 需求 → 自動建立 plan
