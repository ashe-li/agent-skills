...
掃描 `~/.claude/skills/learned/` 目錄（含子目錄），對每個 .md 檔案收集：slug（檔名）、有無 frontmatter、frontmatter 完整度（是否包含 name/description/user-invocable/origin）、品質評分格式（表格 vs 單行 vs 無）、mtime、檔案大小。
...
用 AskUserQuestion 展示分類統計（總數、格式問題各類數量、廢棄標記數量），提供選項：
1. 全部自動修正（格式問題 + 刪除廢棄）
2. 只修正格式，廢棄項目逐一確認
3. 逐一確認所有修正
4. 只查看清單，不修正
