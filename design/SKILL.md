...
8. **風險評估** — 潛在的技術風險和對策
9. **Security / Threat Model**（主動觸發，依 [`rules/security-guidance/skill-integration.md`](../rules/security-guidance/skill-integration.md) 的觸發閘）— 先判斷 plan 是否觸及安全敏感面（認證/輸入/endpoint/DB/反序列化/檔案/shell/SSRF/DOM/加密）：
   - **觸及** → plan 必含「Security / Threat Model」章節，逐條對照 `~/.claude/claude-security-guidance.md`（與 plugin 同一份判準）列出威脅與對策；且實作後步驟（ECC 資源）**須納入 security-reviewer agent**（sonnet）
   - **未觸及** → 明示「無安全敏感面，跳過」，不空列
9. **驗收標準** — 怎樣算完成
...
| 效能 | 已評估效能影響 |
| 安全覆蓋（主動驗證）| 若 plan 觸及安全敏感面：必含 Security / Threat Model 章節、逐條對照 `claude-security-guidance.md`、實作後步驟含 security-reviewer。**逐項核對存在性，非只看「已評估」打勾**；若觸及卻缺章節 → FAIL。未觸及則標 N/A |
...
範例：
- `TICKET-1234 過濾 abstract 中的 URL` → `ticket-1234-url`
...
若使用者選擇「進入 worktree」：
1. 從 plan slug 推導 worktree 名稱（例：`ticket-1234-url`）
...
1. 先跑 normalize 兜底 — 若 planner 輸出非 canonical 格式（`**Step N: title**` / 全形冒號 / 自由文字 Dependencies），自動轉成 /plan-run 可吃的格式。canonical 格式跑下去是 no-op：
   ```bash
   python3 "${AGENT_SKILLS_HOME:-$HOME/Documents/agent-skills}/scripts/plan_runner.py" normalize plans/active/<slug>.md --write
   ```
   若 stderr 顯示 `Wrote normalized plan` → 通知 user 已 normalize（`.bak` 備份在 `plans/active/<slug>.md.bak`）。若顯示 `WARN: Dependencies prose did not yield step IDs:` → 提示 user 該行需手動補 step ID。
2. 初始化 state：
   ```bash
   python3 "${AGENT_SKILLS_HOME:-$HOME/Documents/agent-skills}/scripts/plan_runner.py" init plans/active/<slug>.md
   ```
   若 init 仍回 `No steps found in plan`，看 error payload 的 `hint` 欄位（Python 端會建議怎麼修）。
3. 檢查 stdout 的 `warnings` 欄位——若 range syntax warning 已由 parser 自動展開則可忽略；若是其他 warning 才需處理
4. 提示使用者：
