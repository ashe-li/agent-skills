...
- **不動範圍**: historical 檔案（CHANGELOG、research/、plans/completed/）；官方 `security-guidance@claude-plugins-official` plugin（非 ECC）；`ecc-skill-defer/`（依 D4 過渡保留 —— 作廢 2026-09-09：該 skill 已於 PR #66 移除，本豁免失效）

> **2026-09-09 註記（v3.0.0 前置）**：PR #66（commit c948b32）移除 9 支 skill 後，D4 已作廢、S3.1–S3.3 只保留仍存在的 skill，原 S1.2 與 S2.3 整步及各批次中的已刪 skill 一律移入下方「作廢範圍（不可執行，2026-09-09）」區塊——該區塊不會被 `/plan-run` parser 解析成 step，其餘項目效力不變。
...
| D4 | ecc-skill-defer 去留（作廢 2026-09-09：`ecc-skill-defer` 已於 PR #66 移除，本裁決點失效） | ★(a) 保留標 deprecated，等 harness 端 ECC plugin 處置定案（含 gateguard D1 待辦）；(b) 直接刪除 |
...
4. **不可砍清單**（各 Step 的 read-back 驗收依據）：環境路徑/閾值數字/API 怪癖/差異化定位表/真實踩坑教訓。特別是 `plan-run:27-45,198-241`（parser 契約）、`notion-plan` DOM 事實（原列的 `playwright-human-in-the-loop:13-43,136-142` 已作廢 2026-09-09，見「作廢範圍」區塊）
...
- [ ] **S3.1** — 高削減組精簡
  - Files: plan-archive/SKILL.md, handoff/SKILL.md
  - Agent: `general-purpose`
  - Action: plan-archive L116-194 Hook 安裝教學移出至 `docs/hooks-setup.md`（205→~75）；handoff 四處重複規則收斂+刪「設計原則」覆誦段（191→~145）。各檔保留點依 audit 清單逐項核對
  - Dependencies: []
  - Why: 削減比例最高的一批，無 ECC 耦合，純冗餘。原批次另含三支已於 PR #66 移除的 skill，已移入「作廢範圍」區塊
- [ ] **S3.2** — 中削減組精簡
  - Files: notion-plan/SKILL.md, plan-run/SKILL.md
  - Agent: `general-purpose`
  - Action: notion-plan 刪通用 Markdown 教學、保留全部 DOM/eval 踩坑（314→~210）；plan-run 重複敘述收斂、parser 契約與狀態機原樣保留（247→~165）
  - Dependencies: []
  - Why: 機制保留、文件精簡；plan-run 狀態機經查證非冗餘不動。原批次另含兩支已於 PR #66 移除的 skill，已移入「作廢範圍」區塊；原對 S1.2 的依賴隨該步作廢一併移除
- [ ] **S3.3** — 低削減組精簡
  - Files: worktree/SKILL.md, figma-verify/SKILL.md, evidence-check/SKILL.md
  - Agent: `general-purpose`
  - Action: worktree 刪文末覆誦段（196→~155）；figma-verify 四處重複規則收斂（149→~115）；evidence-check 輕度合併重複表格（196→~165）
  - Dependencies: []
  - Why: 環境事實密度高，精簡空間小，最後做。原批次另含一支已於 PR #66 移除的 skill，已移入「作廢範圍」區塊
...
- [ ] **S4.2** — README.md 全面同步
  - Files: README.md
  - Agent: `general-purpose`
  - Action: skill 總覽表、pipeline 描述、L481-490「ECC Agent 退化警告」表全面改寫為解耦後架構
  - Dependencies: S1.3, S3.3
  - Why: README 是即時架構主文件，最後統一改避免多次返工。原 Action 含依 D4 標 deprecated 的說明改寫、原依賴含 S2.3，兩者皆隨 PR #66 作廢（見「作廢範圍」區塊）
- [ ] **S4.3** — CHANGELOG 新增條目
  - Files: CHANGELOG.md
  - Agent: `general-purpose`
...
以下只是紀錄，**不是 step**：`/plan-run` 的 parser 只把 `- [ ]` / `- [x]` 開頭且帶 step ID 的行當成 step，本區塊的普通條列一律不會被解析。對應的 skill 已於 PR #66（commit c948b32）移除，工作不再執行。

- `verify-evidence-loop` — 原 S1.2 整步（誤標修正）與原屬 S3.2 的精簡工作（401→~200）皆不執行；S1.2 已自 Phase 1 移出，原依賴它的 S3.2 改為無依賴
- `assist` — 原 S2.3 整步（路由表重建，292→~145）不執行；S2.3 已自 Phase 2 移出，原依賴它的 S4.2 改為只依賴 S1.3 與 S3.3
- `curation` — 原屬 S3.1，manifest 儀式改原生 Task（161→~102）不執行
- `triage` — 原屬 S3.1，逐字 bash 腳本壓縮（138→~95）不執行
- `learn-eval-deep` — 原屬 S3.1，重複可靠度陳述合併（159→~105）不執行
- `verify-fix-loop` — 原屬 S3.2，pseudocode 重述刪除與 hard-cap 規則單一來源化（312→~230）不執行
- `playwright-human-in-the-loop` — 原屬 S3.3，刪裝飾引用與報告格式重疊（142→~115）不執行；替換原則 4「三軸分級＋安全底線不可砍」一併失效
- `ecc-skill-defer` — D4 推薦的「保留標 deprecated」失效；「不動範圍」對它的豁免、S4.2 README 說明改寫、驗收條件 1 的 grep 白名單項一併作廢

## 驗收條件

```bash
# 1. hard-runtime 殘留 = 0（白名單只剩 historical 檔案；原過渡期豁免已隨 PR #66 作廢）
grep -rn 'everything-claude-code' --include='*.md' --include='*.sh' ~/Documents/agent-skills \
  | grep -v -e CHANGELOG -e research/ -e plans/completed/ -e plans/archived/
# 預期輸出：空
