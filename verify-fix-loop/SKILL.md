...
allowed-tools: Read, Edit, Write, Bash, Glob, Grep, Agent, AskUserQuestion, TaskCreate, TaskUpdate, TaskList, mcp__playwright__browser_navigate, mcp__playwright__browser_click, mcp__playwright__browser_fill_form, mcp__playwright__browser_type, mcp__playwright__browser_snapshot, mcp__playwright__browser_take_screenshot, mcp__playwright__browser_console_messages, mcp__playwright__browser_network_requests, mcp__playwright__browser_evaluate, mcp__playwright__browser_wait_for, mcp__playwright__browser_press_key, mcp__playwright__browser_select_option, mcp__playwright__browser_hover, mcp__playwright__browser_navigate_back, mcp__playwright__browser_tabs, mcp__playwright__browser_close
...
透過 local Playwright MCP（headed 模式）跑「驗證 → 診斷 → 修正 → 重新驗證」迴圈。每輪驗證以 snapshot + console + network 為證據；若 FAIL 則修正程式碼，下一輪再驗證。**完成 2 輪後（即 Round 3 起每輪）強制 HITL 詢問是否繼續**，避免盲目迭代。
...
> - 目標：[$ARGUMENTS 摘要]
> - 預估：每輪 ~5-10k tokens，hard cap = 5 rounds
...
- Dev server 是 long-running process，自動啟動會佔用 token budget 持續 tail stdout
- Skill 結束時 process 殘留難以回收
- 連線重試 / port conflict 邏輯複雜易出錯
...
- 每輪結束 append 一個 `## Round N` 區塊（diagnosis、files changed、verdict）
...
for n in 1..MAX_ROUNDS:
    # === HITL Gate（第 3 輪起）===
    if n > HITL_AFTER:
        decision = ask_user_continue(n, round_log, current_failures)
        if decision != "continue":
            return early_exit_report(decision)

    # === Phase A: Verify ===
    result = verify(target_url, pass_criteria)
    round_log.append({"round": n, "verdict": result.verdict, ...})
    if result.verdict == "PASS":
        return success_report(n, files_changed, round_log)

    # === Phase B: Diagnose ===
    diagnosis = diagnose(result.snapshot, result.console, result.network)

    # === Phase C: Fix ===
    fix = apply_fix(diagnosis, allowed_paths)
    files_changed.extend(fix.files)
    round_log[-1]["fix"] = fix.summary

    # === Phase D: Wait reload ===
    wait_for_reload(target_url)

return cap_exceeded_report(round_log, files_changed)
```

### Phase A: Verify

對 PASS 條件逐項驗證（checklist-driven，依據：OpenAI dev community 共識 — Checklist > Free-form）：
...
## Step 2: HITL Gate（Round 3 起每輪觸發，HITL_AFTER=2）

完成 2 輪後（即 Round 3 起，`if n > HITL_AFTER`），**進入 Phase A 之前**用 `AskUserQuestion`：
...
`MAX_ROUNDS = 5` 是 hard cap，達上限即使選「繼續」也強制停止並輸出 cap-exceeded 報告。
...
- **永不為過而過** — Fix 的目標是「讓行為符合預期」，不是「讓檢查放水」。Phase C 的禁止清單是硬規則
- **Headed 模式必要性** — 使用者可同步觀察、HITL 時能視覺確認；headless 失去這個價值
- **Allowed paths 是硬邊界** — 超出範圍的修改須回 Step 0b 重新確認，不可越權
- **Hard cap 5 rounds** — 超過代表問題已超出 loop 範圍，須切換到 /design 或人工介入；METR 2025 證據顯示 >3 輪 agent 開始 drift，故設 5 為極限
- **每輪 round_log** — 即使早退，已修改檔案不自動 revert（使用者可 `git diff` / `git restore` 自行決定）
- **不自動 commit** — 迴圈結束後由使用者決定是否 `/pr`
- **不啟動 dev server** — Step 0b 確認狀態，自動啟動長時間佔用且難回收
...
- **arXiv:2509.18970** — 結構性分類（PASS criteria checklist）優先於逐案語意判斷
