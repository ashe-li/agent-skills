...
**跨 repo 批次**：直接呼叫 `scripts/ops/worktree-cleanup.sh`（dry-run 預設；`--fetch --apply` 才動作；`--du` 加算磁碟用量；`--kill-blockers` 連佔用行程一起清；`--root` 覆寫掃描根目錄）。該腳本除了 PR 狀態與工作區乾淨，還會驗「有無未推送 commit」與「有無行程以該 worktree 為 cwd」，並且**永遠不刪 branch**；細節見 `scripts/ops/README.md`。
